...
configuration = build_config_file(True,'LGCN','facebook_book')#cp.open_config(CONFIG_FILE)#basic_conf_file('BPRMF'))#GET CONFIGURATION TEMPLATE
...
def extract_recs_from_raw_data(type,ALG):
    files = raw_files_names_list(DATASET,ALG)
    for f in files:
        if f.split("_")[2] == 'KPCA':
            cp.save_recs(ALG,f, DATASET,type=type,KPCA=True)
        else:
            cp.save_recs(ALG,f, DATASET,type=type)

def clear_results_dir():
    cp.clear_results_dir(DATASET)

def compute_metrics_on_recs(DATASET,ALG):
    configuration = cp.open_config(metrics_configuration_file())
    configuration = set_runtime_metrics_configuration(configuration,DATASET,ALG)
...
def get_results(DATASET,ALG,CSV_NAME):
    extract_recs_from_raw_data('test',ALG)
    clear_results_dir()
    compute_metrics_on_recs(DATASET,ALG)
    results_csv(CSV_NAME)



#get_results(FACEBOOK,'LGCN','LGCN_FA_TEST_')
#get_results(FACEBOOK,'BPRMF','BPRMF_FA_TEST_')

#get_results(YAHOO,'LGCN','LGCN_YA_TEST_')
get_results(YAHOO,'BPRMF','BPRMF_YA_TEST_')

#CHANGE CONFIGURATION PARAMETERS TO SUITE EXPERIMENT
#configuration = set_dataset_configuration(configuration, DATASET,ALG)
...
#extract_recs_from_raw_data(type='test')
#clear_results_dir()
#compute_metrics_on_recs(DATASET)
#results_csv('LGCN_RESULTS_Yah_')
