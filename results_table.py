...
yaml.indent(mapping=4, sequence=6, offset=1)  # not that that looks nice
...
def save_recs(f,DATASET):            
    ...
    for SIZE in recs['validation'].keys():   
        with open('data_dz/'+DATASET+'/'+f.split("_")[2]+'/'+f.split("_")[2]+'@'+str(SIZE)+'.tsv'  ,'w') as file:
...
def save_recs_kpca(f,DATASET):
    ...
    for KERNEL in recs['validation'].keys():
        for SIZE in recs['validation'][KERNEL].keys():
            with open('data_dz/'+DATASET+'/'+f.split("_")[2]+'/'+f.split("_")[2]+'_'+KERNEL+'@'+str(SIZE)+'.tsv'  ,'w') as file:            
...
DATASET = 'yahoo_movies'#['yahoo_movies','facebook_book']
...
with open('custom_configs/custom.yml', 'r') as file:    
    configuration = yaml.load(file)


#DATASET CONFIGURATION
configuration['experiment']['data_config']['train_path']= '../data/'+DATASET+'/train.tsv'
configuration['experiment']['data_config']['validation_path']= '../data/'+DATASET+'/val.tsv'
configuration['experiment']['data_config']['test_path']= '../data/'+DATASET+'/test.tsv'
configuration['experiment']['dataset'] = DATASET
configuration['experiment']['models']['external.LightGCN_Custom']['meta']['dataset_name']  =  DATASET
...
if(BASE):
    for factors in [64,32,16,8,4,2]:
        ...
        configuration['experiment']['models']['external.LightGCN_Custom']['meta']['KPCA_Kernels']  =  ['linear','poly', 'rbf', 'sigmoid', 'cosine']
        configuration['experiment']['models']['external.LightGCN_Custom']['factors'] = factors
        with open('custom_configs/custom.yml', 'w') as file:
...
if(REDUCERS):
    ...
    configuration['experiment']['models']['external.LightGCN_Custom']['meta']['reducers_types']    =  ['UMAP','PCA','KPCA','TSNE']
...
if(METRICS):
    ...
    if (CLEAR_RESULTS):
        files = glob.glob('results/'+DATASET+'/performance/*') #[f for f in os.listdir('results/facebook_book/performance/')]
        for f in files:
            os.remove(f)
    ...
    for type in ['base','kpca','pca','tsne','umap']:
        ...
        configs['experiment']['models']['RecommendationFolder']['folder']= os.path.abspath('data_dz/'+DATASET+'/'+type)
